...
def convert_dl3dv_scene(scene_path, output_path, target_size=448):
    ...
    for frame_idx, frame in enumerate(tqdm(frames)):
        ...
        try:
            ...
            # Get original COLMAP image ID if available
    ...
    # Check if we have colmap_im_id in the data
    has_colmap_ids = any(frame.get('colmap_im_id') is not None for frame in available_frames)
    if has_colmap_ids:
        print("  Using original COLMAP image IDs from transforms.json")
    else:
        print("  ⚠️  No colmap_im_id found in transforms.json, using sequential IDs")
    ...
    for idx, frame in enumerate(available_frames):
        ...
        # Get image ID: prefer colmap_im_id from JSON, fallback to sequential
        colmap_im_id = frame.get('colmap_im_id')
        if colmap_im_id is not None:
            image_id = colmap_im_id
        else:
        ...
        image_data.append({
            'image_id': image_id,  # Use original ID to preserve 3D point associations!
            'qvec': qvec,
            'tvec': t_w2c,
            'camera_id': 1,
