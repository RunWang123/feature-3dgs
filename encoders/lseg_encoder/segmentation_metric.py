...
def get_lsm_palette(num_classes):
    """
    Generate LSM-aligned tab10 colormap palette.
    This matches the color scheme used in the LSM project for semantic segmentation.
    
    Args:
        num_classes: Number of semantic classes
    
    Returns:
        palette: Flat list of RGB values (0-255) in format [R0,G0,B0,R1,G1,B1,...]
    """
    import matplotlib.pyplot as plt
    
    # Use tab10 colormap (same as LSM)
    cmap = plt.cm.get_cmap('tab10', num_classes + 1)
    
    palette = []
    for i in range(num_classes + 1):
        rgb = cmap(i)[:3]  # Get RGB values in 0-1 range
        palette.extend([int(rgb[0] * 255), int(rgb[1] * 255), int(rgb[2] * 255)])
    
    # Extend to match adepalette length if needed
    while len(palette) < len(adepallete):
        palette.append(0)
